 ...
 * - ARTICLE: quy trình cũ (fetch + summarize)
 * Ngoài ra: nếu user đăng nhập → auto-like bài sau khi create/update
 */
router.post("/ingest", requireAuthOptional as any, async (req: any, res) => {
  try {
    ...
    // ===== NHÁNH MEDIA: YouTube/TikTok/Spotify/SoundCloud/Instagram/X =====
    ...
    if (det.provider && det.content_type && det.content_type !== "article") {
      ...
      // Tìm record cũ theo (provider,provider_id) hoặc trùng url/original_url (canonical hoặc raw)
      ...
      // --- AUTO-LIKE nếu có user đăng nhập ---
    ...
    // ===== NHÁNH BÀI VIẾT: giữ quy trình cũ (fetch + summarize) =====
    ...
    if (exist[0] && exist[0].content_hash === contentHash && exist[0].summary_html) {
      ...
      if (req.user?.id && item?.id) {
        try {
          ...
          console.warn("auto-like failed (cached):", e);
        }
      }
    ...
    if (exist[0]) {
      const rows = await q<any>(
        ...
        [
          ...
          contentHash,              // $6
          summaryHtml,              // $7
          keywordsJson,             // $8 (jsonb)
          embeddingStr,             // $9  -> (ở SQL là $10 vì đã dịch cột)
          sourceUsed || "medium"    // $10 -> (ở SQL là $11)
        ]
      );
      item = rows[0];
    } else {
      const rows = await q<any>(
        ...
        [
          ...
          contentHash,              // $7
          summaryHtml,              // $8
          keywordsJson,             // $9  (jsonb)
          embeddingStr,             // $10 (vector from text)
    ...
    if (req.user?.id && item?.id) {
      try {
        ...
        console.warn("auto-like failed (article):", e);
      }
    }
    // --- END AUTO-LIKE ---
